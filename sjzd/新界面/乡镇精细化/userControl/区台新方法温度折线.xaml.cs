...
namespace sjzd
{
    ...
    /// 个人评分页.xaml 的交互逻辑
    /// </summary>
    public partial class 区台新方法温度折线 : UserControl
    {
        ...
        public 区台新方法温度折线(List<mysql数据库类.区台温度> temLists)
        {
            InitializeComponent();
            ...
            foreach (mysql数据库类.区台温度 item in temLists)
            {
                if (item.TEM > tmax)
                    tmax = item.TEM;
                if (item.TEM < tmin)
            ...
            foreach (mysql数据库类.区台温度 item in temLists)
            {
                if (item.TEM == tmax || item.TEM == tmin)
                {
                    ...
                    {
                        Category = item.MyDate.ToString("d日H时"),
                        Value = Math.Round(item.TEM, 2),
                        Label = item.MyDate.ToString("d日H时：") + Math.Round(item.TEM, 2) + "℃"
                    });
                }
                else
                {
