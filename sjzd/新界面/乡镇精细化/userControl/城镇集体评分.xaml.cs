...
namespace sjzd
{
    ...
    /// </summary>
    public partial class 城镇集体评分 : UserControl
    {
        private SplashScreenDataContext splashScreenDataContext;
        private readonly ObservableCollection<城镇预报集体评分ViewModel> 评分列表 = new My城镇集体评分ViewModel().Clubs;
        private string xlsPath = "";
        public 城镇集体评分()
        {
            InitializeComponent();
            BTLabel.Content = "城镇集体评分";
        ...
        private void CXButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if ((sDate.SelectedDate.ToString().Length != 0) && (eDate.SelectedDate.ToString().Length != 0))
            {
                ...
                if ((sDate.SelectedDate.ToString().Length != 0) && (eDate.SelectedDate.ToString().Length != 0))
                {
                    ...
                    BTLabel.Content = startDate + "至" + endDate + gwchoose.Text + "城镇集体评分";
                    //gwchoose.Text
                    ...
                    for (int i = 0; i < intQXGS + 1; i++)
                    {
                        if (i < intQXGS)
                        {
                            ...
                            XSList[0] = (float)Math.Round((zqlFloat[2] * 10 + zqlFloat[5] * 8 + zqlFloat[8] * 6 + zqlFloat[11] * 2 + zqlFloat[14] * 1) / 27, 2);
                            XSList[1] = (float)Math.Round((zqlFloat[0] * 10 + zqlFloat[3] * 8 + zqlFloat[6] * 6 + zqlFloat[9] * 2 + zqlFloat[12] * 1) / 27, 2);
                            XSList[2] = (float)Math.Round((zqlFloat[1] * 10 + zqlFloat[4] * 8 + zqlFloat[7] * 6 + zqlFloat[10] * 2 + zqlFloat[13] * 1) / 27, 2);
                            ...
                            XSList[4] = (float)Math.Round((QYJQ[0] * 10 + QYJQ[1] * 8 + QYJQ[2] * 6 + QYJQ[3] * 2 + QYJQ[4] * 1) / 27, 2);//晴雨技巧
                            XSList[5] = (float)Math.Round((WDJQ[0] * 10 + WDJQ[2] * 8 + WDJQ[4] * 6 + WDJQ[6] * 2 + WDJQ[8] * 1) / 27, 2);//高温技巧
                            XSList[6] = (float)Math.Round(WDJQ[1] * 10 + WDJQ[3] * 8 + WDJQ[5] * 6 + WDJQ[7] * 2 + WDJQ[9] * 1 / 27, 2);//低温技巧
        ...
        private void DCButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.DialogResult == true)
            {
                ...
                try
                {
                    ...
                    cellSheet.Cells[0, 36].PutValue("120小时最低温度技巧");
                    cellSheet.Cells[0, 37].PutValue("120小时最高温度技巧");
                    cellSheet.Cells[0, 38].PutValue("120小时晴雨技巧");
                    for (int i = 0; i < dcsz.Length; i++)
                    {
                        ...
                        cellSheet.Cells[i + 1, 36].PutValue(Math.Round(dcsz[i].SJ120TminJq, 2));
                        cellSheet.Cells[i + 1, 37].PutValue(Math.Round(dcsz[i].SJ120TmaxJq, 2));
                        cellSheet.Cells[i + 1, 38].PutValue(Math.Round(dcsz[i].SJ120Qyjq, 2));
                        for(int j = 9; j < 39; j++)
                        {
    ...
    public class My城镇集体评分ViewModel : ViewModelBase
    {
