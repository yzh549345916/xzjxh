...
namespace sjzd
{
    ...
    /// </summary>
    public partial class 旗县乡镇精细化预报准确率页new : UserControl
    {
        private readonly ObservableCollection<旗县乡镇精细化预报准确率ViewModel> 准确率列表 = new My旗县乡镇精细化准确率ViewModel().Clubs;
        private string xlsPath = "";
        private SplashScreenDataContext splashScreenDataContext;

        public 旗县乡镇精细化预报准确率页new()
        {
            InitializeComponent();
            BTLabel.Content = "旗县乡镇精细化准确率查询";
        ...
        private void CXButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!(sDate.SelectedDate.ToString().Length == 0) && !(eDate.SelectedDate.ToString().Length == 0))
            {
                ...
                BTLabel.Content = startDate + "至" + endDate + "各旗县乡镇精细化准确率查询";
                ...
                for (int i = 0; i < intQXGS; i++)
                {
                    ...
                    准确率列表.Add(new 旗县乡镇精细化预报准确率ViewModel(QXName[i], zqlFloat[0], zqlFloat[1], zqlFloat[2], zqlFloat[3],
                        zqlFloat[4], zqlFloat[5], zqlFloat[6], zqlFloat[7], zqlFloat[8]));
                }
            }
            else
            {
        ...
        private void DCButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.DialogResult == true)
            {
                ...
                旗县乡镇精细化预报准确率ViewModel[] dcsz = 准确率列表.ToArray();
                try
                {
    ...
    public class My旗县乡镇精细化准确率ViewModel : ViewModelBase
    {
        private ObservableCollection<旗县乡镇精细化预报准确率ViewModel> clubs;

        public ObservableCollection<旗县乡镇精细化预报准确率ViewModel> Clubs
        {
            get
            {
        ...
        private ObservableCollection<旗县乡镇精细化预报准确率ViewModel> CreateClubs()
        {
            ObservableCollection<旗县乡镇精细化预报准确率ViewModel> clubs = new ObservableCollection<旗县乡镇精细化预报准确率ViewModel>();
            return clubs;
        }
    }
}