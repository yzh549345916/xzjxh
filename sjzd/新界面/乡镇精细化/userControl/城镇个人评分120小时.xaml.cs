...
namespace sjzd
{
    ...
    /// 个人评分页.xaml 的交互逻辑
    /// </summary>
    public partial class 城镇个人评分120小时 : UserControl
    {
        ...
        private void CXButton_Click(object sender, RoutedEventArgs e)
        {
          ...
          try
          {
                if ((!(sDate.SelectedDate.ToString().Length == 0)) && (!(eDate.SelectedDate.ToString().Length == 0)))
                {
                    ...
                    BTLabel.Content = startDate + "至" + endDate + QXSelect.Text + "城镇预报120小时个人评分查询";
                    ...
                    if (peopleStr.Length > 0)
                    {
                        ...
                        for (int i = 0; i < peoplesz.Length; i++)
                        {
                            ...
                            float[] zqlFloat = tj.GRZQL120(Convert.ToDateTime(sDate.SelectedDate), Convert.ToDateTime(eDate.SelectedDate), GRPFSZ[i, 0]);//返回数组分别为五天预报的最高、最低温度、晴雨准确率以及缺报率
                            GRPFSZ[i, 5] = Convert.ToString(Math.Round((zqlFloat[2] * 10 + zqlFloat[5] * 8 + zqlFloat[8] * 6 + zqlFloat[11] * 2 + zqlFloat[14] * 1) / 27, 2));
                            GRPFSZ[i, 6] = Convert.ToString(Math.Round((zqlFloat[0] * 10 + zqlFloat[3] * 8 + zqlFloat[6] * 6 + zqlFloat[9] * 2 + zqlFloat[12] * 1) / 27, 2));
                            GRPFSZ[i, 7] = Convert.ToString(Math.Round((zqlFloat[1] * 10 + zqlFloat[4] * 8 + zqlFloat[7] * 6 + zqlFloat[10] * 2 + zqlFloat[13] * 1) / 27, 2));
                            GRPFSZ[i, 8] = Convert.ToString(Math.Round(Convert.ToSingle(0.2 * Convert.ToDouble(GRPFSZ[i, 5]) + 0.4 * Convert.ToDouble(GRPFSZ[i, 6]) + 0.4 * Convert.ToDouble(GRPFSZ[i, 7])), 2));//总评分
                            float[] GRJDWCSZ = tj.GRJDWC120(Convert.ToDateTime(sDate.SelectedDate), Convert.ToDateTime(eDate.SelectedDate), GRPFSZ[i, 0]);
                            float[] ZDJDWCSZ = tj.GRZDJDWC120(Convert.ToDateTime(sDate.SelectedDate), Convert.ToDateTime(eDate.SelectedDate), GRPFSZ[i, 0]);
                            float[] ZDzqlFloat = tj.GRZYZQL120(Convert.ToDateTime(sDate.SelectedDate), Convert.ToDateTime(eDate.SelectedDate), GRPFSZ[i, 0]);//返回数组分别为中央指导五天预报的最高、最低温度、晴雨准确率以及缺报率，主要计算技巧用晴雨准确率

                            float[] WDJQ = new float[10];//保存五天的最高、最低温度技巧
                            try
                            {
                                for (int j = 0; j < 10; j++)
                                {
                            ...
                            try
                            {
                                for (int j = 0; j < 5; j++)
                                {
                                    if (Math.Abs(ZDzqlFloat[2 + j * 3] - 100) < 0.00001)
                                    {
                                        if (Math.Abs(zqlFloat[2 + j * 3] - ZDzqlFloat[2 + j * 3]) > 0.00001)
                                        {
    ...
    public class My城镇精细化个人评分ViewModel : ViewModelBase
    {
        private ObservableCollection<城镇预报120小时个人评分ViewModel> clubs;

        public ObservableCollection<城镇预报120小时个人评分ViewModel> Clubs
        {
            get
            {
        ...
        private ObservableCollection<城镇预报120小时个人评分ViewModel> CreateClubs()
        {
            ObservableCollection<城镇预报120小时个人评分ViewModel> clubs = new ObservableCollection<城镇预报120小时个人评分ViewModel>();
            return clubs;
        }
    }
}
