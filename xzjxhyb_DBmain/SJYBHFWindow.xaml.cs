...
{
    /// <summary>
    /// SJYBHFWindow.xaml 的交互逻辑
    /// </summary>
    public partial class SJYBHFWindow : Window
    {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            if ((!(sDate.SelectedDate.ToString().Length == 0)) && (!(eDate.SelectedDate.ToString().Length == 0)))
            {
                ...
                for (int i = 0; DateTime.Compare(dateLS, dateEndDate) <= 0; i++)//临时日期初始值为开始日期，每个循环加1天，一直到大于截止日期
                {
                    ...
                    try
                    {
                        SJZDCXRK sjzdcxrk = new SJZDCXRK();
                        string[,] YBSZ = sjzdcxrk.ZDYBCL(sjzdcxrk.readXZYBtxt(dateLS));
                        string strQXSK = sjzdcxrk.CIMISSHQQXSK(dateLS);
                        string strXZSK = sjzdcxrk.CIMISSHQXZSK(dateLS, strQXSK, ref strError);
                        string[,] szYB = sjzdcxrk.CZCL(YBSZ, strQXSK, strXZSK, ref strError);
                        sjzdcxrk.ZDSZ2BW(dateLS, szYB);
                        sjzdcxrk.centerBW2ZDBW(dateLS);
                        sjzdcxrk.ZDYBBWtoSZ(dateLS, strDate);
                        saveSJYB savesjyb = new saveSJYB();

                        ss += strDate + "市局指导预报成功恢复" + '\n';
                        ss += savesjyb.saveSJCS(dateLS.ToString("yyyyMMdd"));
                    }
                    catch (Exception ex)
                    {
                        ss += ex.Message + '\n';
